...
public class Writer {
  public static void write(/* List<Admin> admins, */ List<User> users, List<Book> books) {
    try {
      ...
      om.writerWithDefaultPrettyPrinter().writeValue(new File("src/main/resources/users.json"), users);
      om.writerWithDefaultPrettyPrinter().writeValue(new File("src/main/resources/books.json"), books);
